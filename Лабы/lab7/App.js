import './App.css';
import React from 'react';
import Notes from './notes'
//бросаем map на верх и точка

//const weekdays =["" ]

function App() {

   
  return (
    
    <Sheduler/>
  );
  
}

function Sheduler(){return <Calendar/>}

export default App;


class Calendar extends React.Component 
{
  
  constructor(props){
    super(props)
    this.props={}
    this.state={ndate:new Date(),thisdate:1,adat:new Date(),click:false,notesArr:new Map()}
    this.sw_month=this.sw_month.bind(this)
    this.selday=this.selday.bind(this)
    this.clearall=this.clearall.bind(this)
    this.update_buff=this.update_buff.bind(this)
  }
  
  render()
  {//this.state.notesArr.has()
     return(
   <React.Fragment>
   <table>
    
    <CalendarHead dat={this.state.ndate} eventh={this.sw_month} />
    <CalendarBody dat={this.state.ndate} notesArr={this.state.notesArr} adat={this.state.adat} eventh2={this.selday}/>
    
   </table>
   {this.state.notesArr.get(this.state.adat.toString()) /* <Notes></Notes> */}
   </React.Fragment>
    )
  }

  update_buff(notes_a=[])
  {
    let buffmap = this.state.notesArr
    //buffmap.delete(this.state.adat.toString())
    buffmap.set(this.state.adat.toString(), <Notes update={this.update_buff} notes={notes_a} key={new Date().getTime().toString()+"notes"} iremover={this.clearall}/>)
    
    this.setState({notesArr:buffmap})
  }

  clearall(handler)
  {
    let buff= this.state.notesArr
    buff.delete(this.state.adat.toString())
    this.setState({notesArr:buff})
  }

  selday(handler)
  { 
    let buffmap=new Map()
    let examp = new Date(this.state.ndate)
    examp.setDate(Number(handler.target.innerText))
    
    if(this.state.adat.getDate()==examp.getDate()){this.state.click=true}
    else{this.state.click=false}
    this.setState({adat:examp})
    if(this.state.click&&!this.state.notesArr.has(examp.toString()))
    {
      console.log(22)
      buffmap = this.state.notesArr
      buffmap.delete(examp.toString())
      buffmap.set(examp.toString(), <Notes update={this.update_buff} notes={[]} key={new Date().getTime().toString()+"notes"} iremover={this.clearall}/>)
      
      this.setState({notesArr:buffmap})
    }
    //console.log(handler.target.innerText)
    //console.log(this.state.ndate.getMonth())
    //this.forceUpdate()
  }

  sw_month(handler)
  {
    let examp =this.state.ndate
    //console.log(handler.target.value)
    if(handler.target.value=="-1")//inverce
    {
      {examp.setMonth(examp.getMonth()-1)}
    
    }
    else//1
    {
      {examp.setMonth(examp.getMonth()+1)}
     
    }
    this.setState({ndate:examp})
   
  }
  
  

}

class CalendarHead extends React.Component 
{
  constructor(props){
    super(props)
    this.props={}
    this.state={}
  }
  render()
  {
    const monthNames = ["Январь", "Февраль", "Март", "Апрель", "Май", "Июнь",
  "Июль", "Август", "Сентябрь", "Октябрь", "Ноябрь", "Декабрь"
];

     return(
    <React.Fragment>
    
    <tr> <td colSpan={7}>{monthNames[this.props.dat.getMonth()]} {this.props.dat.getFullYear()}  <button value={-1} onClick={this.props.eventh}> l </button> <button value={1} onClick={this.props.eventh}> r </button> </td></tr>
    <tr><td>Пн</td><td>Вт</td><td>Ср</td><td>Чт</td><td>Пт</td><td>Сб</td><td>Вс</td></tr>
    </React.Fragment>
    )
  }

}
class CalendarBody extends React.Component 
{
  constructor(props){
    super(props)
    this.props={}
  }

  

  render()
  { 
    
    const datcon =this.props.dat
    //this.setState({tday:datcon.getDay(),ldate:datcon.getDate()})
    let diff =getfday(datcon.getMonth(),datcon.getFullYear())
    if(diff>0)
    {
     diff--
    }
    else{diff=6}
    let fullweek=[]
    let max =daysInMonth(datcon.getMonth()+1,datcon.getFullYear())
    let monthl=1
    let mon2=1
    let datebuff=new Date(datcon)
    
    for(let it1=0;it1<6;it1++)
    {
      let weekdays=[]
      for(let it2=0;it2<7;it2++)
      {
        datebuff.setDate(monthl)
        //console.log(datcon)
        if(diff!=0){weekdays.push(
          <td class={"past"} >{daysInMonth(datcon.getFullYear(),datcon.getMonth())-diff }</td>
         )
        diff--;}
         else{
          if(max>=monthl)
          { if(this.props.adat.getDate()==monthl){weekdays.push(<td class={"select"} onClick={this.props.eventh2}>{monthl++}</td>)}
            else{
            if((new Date().getDate())==monthl && (datcon.getMonth() == new Date().getMonth()) && (
              (datcon.getFullYear() == new Date().getFullYear())
            )){  weekdays.push(<td class={"today"} onClick={this.props.eventh2}>{monthl++}</td>)}
            
            else if(this.props.notesArr.has( datebuff.toString()))
            {
              weekdays.push(<td class={"mark"} onClick={this.props.eventh2}>{monthl++}</td>)
            }
            else
            {
              weekdays.push(<td onClick={this.props.eventh2}>{monthl++}</td>)
            }
            }
          }
          else{

            weekdays.push
            (
              <td class={"past"} >{mon2++}</td>
             )

          }
          }
      }
      fullweek.push(<tr>{weekdays}</tr>)
    }
    
    
    return(
   
    

   <React.Fragment>
    {fullweek}
    </React.Fragment>
    )
  }

}

function daysInMonth (month, year) {
  return new Date(year, month, 0).getDate();
}
function getfday (month, year) {
  return new Date(year, month, 1).getDay();
}



